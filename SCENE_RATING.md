...
|---|-----------|------|-------|---------------|
| 1 | `Kitchen_TiagoCompatible.usda` | **S** | 9.5/10 | Full kitchen: countertops, sink, fridge, shelves at reachable heights. Maximum diversity of pick-place tasks. Tested and validated with all intents. |
...
| 6 | `Fridge_Zil_TiagoCompatible.usda` | B | 5.5/10 | Soviet-era fridge model. Non-standard proportions, unusual handle geometry. Niche visual domain that would skew dataset distribution. |
| 7 | `Dish_set_bowl_cup_mug_spoon_teapot_vase.._TiagoCompatible.usda` | C | 4.0/10 | Props-only scene, no surrounding environment. Objects float on a flat plane. No surfaces/containers for meaningful pick-place context. Spawn geometry conflicts with Tiago reach envelope. |
| 8 | `60s_Office_Props_TiagoCompatible.usda` | C | 3.5/10 | Retro office furniture, no kitchen/manipulation surfaces. Objects are primarily decorative (lamps, typewriter). Minimal overlap with target task distribution. |
| 9 | `Office_Interactive.usd` | C | 3.0/10 | Generic office — desks, chairs, monitors. No kitchen appliances or food-type objects. Not relevant for manipulation tasks in the current task set (pick, fridge, sink). |

## Selection Criteria

1. **Task coverage** — scene must support ≥2 of the 3 core intents (`plan_pick_sink`, `plan_pick_fridge`, `open_close_fridge`)
