...
foreach ($scene in $scenes) {
    Write-Host ""
    Write-Host "============================================================"
    Write-Host "[RUN] strict scene-fit: $scene"
    Write-Host "============================================================"

    $fitArgs = @(
        "scripts/scene_fit_validator.py",
        "--scene-usd", $scene,
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "[FAIL] scene-fit failed: $scene" -ForegroundColor Red
        $failed += $scene
    } else {
        Write-Host "[OK] scene-fit passed: $scene" -ForegroundColor Green
    }
}

Write-Host ""
if ($failed.Count -gt 0) {
    Write-Host "[SUMMARY] Scene-fit failures:" -ForegroundColor Red
    $failed | ForEach-Object { Write-Host " - $_" -ForegroundColor Red }
    exit 1
}

Write-Host "[SUMMARY] All strict scene-fit validations passed." -ForegroundColor Green
exit 0
